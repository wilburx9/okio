...
val CommonBufferedSourceTest by testSuite {
  for (factory in BufferedSourceFactory.entries) {
    testSuite(factory.name) {
      testFixture { CommonBufferedSourceFixture(factory) } asContextForEach {
        test("readBytes") {
        ...
        test("longDecimalLoneDashThrows") {
          sink.writeUtf8("-")
          sink.emit()
          assertFailsWith<EOFException> {
            source.readDecimalLong()
        ...
        test("longDecimalDashFollowedByNonDigitThrows") {
          sink.writeUtf8("- ")
          sink.emit()
        ...
        test("selectSpanningMultipleSegments") {
          if (factory.isOneByteAtATime && isBrowser()) {
            return@test // This test times out on browsers.
          }
        ...
        test("rangeEqualsOnlyReadsUntilMismatch") {
          if (factory !== BufferedSourceFactory.OneByteAtATimeSource) return@test // Other sources read in chunks anyway.
