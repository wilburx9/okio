...
// TODO: Investigate commented freezing test
val PipeTest by testSuite {
  testFixture { PipeFixture() } asContextForEach {
    test("test") {
      val pipe = Pipe(6)
...
//    }
    ...
    test("sinkWriteFailsByClosedReader") {
      val pipe = Pipe(3L)
    ...
    test("sinkFlushDoesntWaitForReader") {
      val pipe = Pipe(100L)
    ...
    test("sinkFlushFailsIfReaderIsClosedBeforeAllDataIsRead") {
      val pipe = Pipe(100L)
      pipe.sink.write(Buffer().writeUtf8("abc"), 3)
      pipe.source.close()
      try {
    ...
    test("sinkCloseFailsIfReaderIsClosedBeforeAllDataIsRead") {
      val pipe = Pipe(100L)
      pipe.sink.write(Buffer().writeUtf8("abc"), 3)
      pipe.source.close()
      try {
    ...
    test("sinkMultipleClose") {
      val pipe = Pipe(100L)
      pipe.sink.close()
      pipe.sink.close()
    }

    test("sinkCloseDoesntWaitForSourceRead") {
      val pipe = Pipe(100L)
      pipe.sink.write(Buffer().writeUtf8("abc"), 3)
      pipe.sink.close()
      val bufferedSource = pipe.source.buffer()
    ...
    test("sourceMultipleClose") {
      val pipe = Pipe(100L)
      pipe.source.close()
      pipe.source.close()
    }

    test("sourceReadUnblockedByClosedSink") {
      val pipe = Pipe(3L)
      executorService.schedule(1000.milliseconds) {
        pipe.sink.close()
      }
...
private class PipeFixture(): AutoCloseable {
